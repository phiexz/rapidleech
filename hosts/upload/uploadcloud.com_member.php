...
$not_done=true;
$continue_up=false;
if ($upc_login & $upc_pass){
	$_REQUEST['login'] = $upc_login;
	$_REQUEST['password'] = $upc_pass;
...
<tr><td align=center>
<div id=login width=100% align=center>Login to www.uploadcloud.com</div>
<?php
			$post['act'] = "login" ;
			...
			$page = geturl("www.uploadcloud.com", 80, "/login.php", 0, 0, $post, 0, $_GET["proxy"], $pauth);
			is_page($page);
			...
			$page = geturl("www.uploadcloud.com", 80, "/members.php", "http://www.uploadmachine.com/", $cookies, 0, 0, "");
			is_page($page);
...
<?
	...
	$Url=parse_url('http://uploadcloud.com/members.php?showlinks=1');
	$page = geturl($Url["host"],  80, $Url["path"].($Url["query"] ? "?".$Url["query"] : ""), 'http://uploadcloud.com/', $cookies, 0, 0, $_GET["proxy"],$pauth);
	is_page($page);
	
	$temp=cut_str($page,'this text">'.$lname,' target=blank>http://uploadcloud.com/delete.php?');
	$ddl=cut_str($page,'http://uploadcloud.com/file/','"');
	$del=cut_str($page,'http://uploadcloud.com/delete.php?id=','"');
	
	$download_link= 'http://uploadcloud.com/file/'.$ddl;
	$delete_link= 'http://uploadcloud.com/delete.php?id='.$del;
	
	}
// Made by Baking 13/07/2009 17:57
