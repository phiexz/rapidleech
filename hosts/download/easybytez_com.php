...
if (!file_exists(HOST_DIR . 'download/GenericXFS_DL.php')) html_error('Cannot load "'.htmlentities(HOST_DIR).'download/GenericXFS_DL.php" 
(File doesn\'t exists)');
...
class easybytez_com extends GenericXFS_DL {
    ...
    protected function checkLogin($page) {
        if (preg_match('@Your IP is temporarily blocked due to \d+ incorrect login attempts<br(?:\s*/)?>This block will last for \d+ \w+.@i', 
$page, $err)) html_error($err[0]);
        return parent::checkLogin($page);
    }
    // It always show expire date... So i will have to check it.
    protected function isAccPremium($page) {
        if (!preg_match('@Premium account expire:\s*</TD>\s*<TD>\s*<b>\s*(\d+ \w+ \d{4})\s*</b>@i', $page, $date) || !($date = 
strtotime($date[1]))) html_error('Cannot check premium status.');
